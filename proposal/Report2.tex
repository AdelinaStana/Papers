...
In our experiments the filter based on commit size had a hard threshold of 10 files, meaning that we use co-changing pairs only from commits with at most 10 files changed. The filter based on connection strength had a variable threshold, starting with 10\% and ending with 100\%, meaning that for 100\% in all commits the two entities that form a co-changing pair are together. 

In section \ref{sec:current_measurements} we used logical dependencies together with structural dependencies in order to detect key classes. We also used only logical dependencies to detect key classes.

Based on the results obtained we did saw an improvement in key classes detection when both logical and structural dependencies were used together. When we used only logical dependencies to detect key classes the results were not bad but were less performing than using only structural or structural and logical dependencies combined. 
