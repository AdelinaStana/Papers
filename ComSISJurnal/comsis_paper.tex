...
The concept of logical coupling (dependency) was first introduced by Gall \cite{Gall:1998:DLC:850947.853338}. A logical dependency between two software entities (classes, modules, interfaces, etc. ) signals that those entities in some way depend on one another. They defined the logical dependency between two modules as the fact that the modules repeatedly change together during the historical evolution of the software system.

%%%%

Structural dependencies do not co-evolve, and vice-versa \cite{Oliva:2011:ISL:2067853.2068086}, \cite{DBLP:conf/issre/OlivaG15}, \cite{DBLP:journals/jss/AjienkaC17}

Lanza et all consider that LD might be useful because can reveal dependencies that are not visible via code analysis \cite{inproceedings_radar_evolution}


Previous research uses the frequency and the confidence measures \cite{Zimmermann:2004:MVH:998675.999460}, \cite{DBLP:journals/jss/AjienkaC17}, \cite{Yu2007}. Zimmermann et al use Apriori algorithm to filter the co-changes, but it took several days to complete the experiments.
