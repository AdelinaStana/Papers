...
We defined the logical dependencies as filtered co-changing pairs extracted from the versioning system history. In  The filters applied to the co-changing pairs are the following: the filter based on commit size and the filter based on connection strength.

The filter based on connection strength had a variable threshold, starting with 10 and ending at 100. We used a variable threshold for connection strength because we wanted to observe how this threshold will impact the key classes detection.
