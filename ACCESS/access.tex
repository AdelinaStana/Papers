...
\begin{abstract}  
Version control systems are important for tracking and managing changes in software code, but they also offer information about the relationships between software entities. When multiple software entities (e.g., classes, interfaces) are modified together, it may indicate an underlying connection. These code co-changes can be filtered to obtain logical dependencies (LD), which represent a type of software dependency that can complement structural or lexical dependencies to improve the understanding of a system's architecture.  

In this paper, we explore the use of logical dependencies extracted from code co-changes as input for software clustering for architectural reconstruction. Since structural dependencies (SD) are the most commonly used dependencies in software clustering, we investigate whether integrating both logical and structural dependencies provides better results than using either dependency type alone.  

Our experiments are conducted on four open-source Java projects from GitHub. We apply three distinct clustering algorithms (Louvain, Leiden, and DBSCAN) and evaluate their performance using two clustering evaluation metrics. Our results show that using logical and structural dependencies as inputs for software clustering improves the quality of software architecture reconstruction. Also, the experiments show that when structural dependencies cannot be extracted, then using only logical dependencies as input for software clustering enables architecture reconstruction with good results.  
\end{abstract}  
...
The system factor is multiplied by the calculated confidence metric value, then scaled by 100. Finally, the results are clipped between 0 and 100.
...
In the second scenario, we run the tool ten times, using only logical dependencies as input. Each run corresponds to a different strength threshold value applied when filtering co-changes during logical dependency extraction. We generate a new logical dependency file for each threshold value, starting with a threshold of 10 and increasing in increments of 10, up to and including 100, where 100 is the maximum value for the threshold. This results in ten separate CSV files, each containing a different subset of logical dependencies, which are then used as input for clustering.
