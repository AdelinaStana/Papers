...
Among the dependencies that can be used for software clustering are structural dependencies (relationships between entities based on code analysis), lexical dependencies (relationships based on naming conventions), and code co-changes (raw relationships between entities extracted from the version control system) / logical dependencies (filtered co-changes), and others.

This paper assesses the impact of logical dependencies in software clustering when used alone and in combination with structural dependencies. Although previous works have used co-changes alone or together with other dependency types for clustering \cite{b18, b16}, our goal is to evaluate whether filtering those co-changes has a meaningful impact on the clustering results. Mainly, we investigate whether filtering, even though it may involve additional computational cost depending on the chosen metrics for filtering, actually improves the obtained clustering quality and how strict the filtering needs to be to achieve this.

In our approach, structural dependencies are used as they are extracted from static code analysis, while the logical dependencies are filtered co-changes obtained from the version control system \cite{b15}. Co-changes are filtered to improve their reliability by removing noise introduced by large commits with many unrelated changes (such as formatting or configuration updates) and low-frequency co-changes that are less likely to reflect meaningful dependencies. Co-changes are filtered based on two metrics: commit size and a metric derived from the confidence metric introduced by Zimmermann et al. \cite{Zimmermann:2004:MVH:998675.999460}, which has been used in practice by other authors for various software engineering tasks \cite{article-Kagdi-commit, Mandal-clones}.
The second metric called the connection strength metric, was also used in our previous research to improve key class detection \cite{b1, b4}.
...
(1) We evaluate the use of logical dependencies in software clustering. Logical dependencies are derived by filtering co-changes using a commit size metric with a fixed threshold and a confidence-based strength metric with multiple threshold values to observe how different filtering thresholds affect clustering quality.
(2) We investigate the impact of combining structural and logical dependencies since logical dependencies can capture relationships that structural dependencies may miss.
(3) We apply three clustering algorithms and evaluate the results using two complementary metrics, MQ and MoJoFM.
(4) We analyze how logical dependencies perform across different strength thresholds and discuss their potential use in systems where structural dependencies cannot be extracted.
