...
\begin{tabular}{|c|l|}
\hline
\textbf{Weight} & \textbf{Dependency categories} \\ 
\hline
...
1 & Local variable \\ 
 \hline
\end{tabular}
\caption{Weights assigned to different dependency types.}
\end{table}
...
We conducted an additional experiment focused on the impact of logical dependency weights on the final results. In this experiment, we increased the weight assigned to each logical dependency for Apache Tomcat to observe the changes it produces in the MQ and MoJoFM metrics.

Table \ref{tab:systems_weights} presents the average dependency weights for all the projects used. The first row contains the average weights for structural dependencies, while the other rows show the average weights for logical dependencies with different strength thresholds.
...
Table \ref{tab:clustering_results_multiplication} contains most of the columns used for the main experiments. In addition, we added two more columns:
\begin{itemize}
    \item \textbf{Multiplication Factor:} The value by which each logical dependency is multiplied.
...
\hline
\end{tabular}
\caption{Comparison of different clustering methods for multiplication factors applied for LD(100) for Tomcat}
