...
In the sections below, we analyze each project in detail and address the research questions.
...
In Table \ref{tab:systems_weights}, which presents the average weights associated with the dependencies across all systems, we can see that for Tomcat, the average weight for LD(10) is already almost double than SD average weight. For LD(100), the average weight is approximately five times higher than that of SD. 
...
Hibernate also has the lowest overlap percentage between LD and SD, as shown in Figure \ref{fig:hibernate_correlation}. Similar to the other systems, the performance for MQ and MoJoFM decreases for SD+LD as the strength threshold becomes stricter.
...
Based on the results from Table \ref{tab:clustering_results_gson}, the SD+LD combination with a strength threshold of 10 achieved the best results for both MQ and MoJoFM. Similar to Apache Ant and Tomcat, all SD+LD combinations achieve better MQ values than SD-only.
...
The effectiveness of combining SD and LD is influenced by the filtering thresholds applied to logical dependencies. Lower strength thresholds (10\% to 40\%) resulted in better MQ and MoJoFM values across all clustering algorithms compared with higher thresholds.

At higher strength thresholds, the MQ and MoJoFM results decline. This decline occurs because the amount of logical dependencies is significantly reduced by the stricter strength thresholds. This reduction leads to fewer new relationships being introduced into the clustering process. Based on our previous research on the overlap between SD and LD, we observed that higher strength thresholds correlate with increased overlap between the two types of dependencies. This overlap indicates that at higher strength thresholds, not much new information is added to the system besides what is already introduced by structural dependencies, reducing the impact of logical dependencies on clustering results.
...
In the case of Hibernate, a stricter strength threshold was needed to achieve the best metric values compared to other systems. Although at a 10\% strength threshold for LD, the metrics obtained are better than SD-only, the system reached peak metric values at a 40\% threshold across all algorithms.

Hibernate, with 16,609 commits and 4,414 entities, has a significantly lower average number of commits per entity than Ant (14,917 commits and 517 entities) or Tomcat (22,698 commits and 662 entities). This lower ratio means that each entity in Hibernate is, on average, involved in fewer commits. As a result, the co-change data extracted for logical dependencies are sparser and contain more noise at lower strength thresholds.

Applying a stricter strength threshold (e.g., 40\%) filters out these weaker logical dependencies.
...
On the other hand, the metric results for LD(10) are, in most cases, still better than those for SD-only and SD+LD. For Apache Ant and Tomcat, LD(10) covers more than 60\% of the system, while for Hibernate and Gson, the coverage is slightly above 30\%. 

Therefore, LD(10) can serve as an alternative to SD-only or SD+LD, especially if structural dependencies are not available from the system.

However, it's important to consider that the performance of LD-only at higher strength thresholds may vary depending on the system's characteristics, such as the number of commits and the number of entities. For example, in the case of Gson, the performance at a 100\% strength threshold is not as good as in other systems, reaching its peak at a 40\% threshold. This is due to the lower number of entities remaining at higher thresholds, with only 18 entities at the 80\% to 100\% strength thresholds, and the relatively small number of commits considered.

In summary, while LD-only can produce good clustering results, especially at higher strength thresholds, its limited coverage reduces its usability, as the clustering is intended for the entire system, not just a small subset. At lower strength thresholds, LD-only offers a good alternative to SD-only, providing acceptable coverage.
...
For the combination of SD and LD (SD+LD), the best performance was generally observed with strength thresholds between 10\% and 40\%. At these thresholds, the clustering solutions achieved higher MQ and MoJoFM values compared to using SD alone.

Selecting the appropriate filtering threshold is important. Lower thresholds include more logical dependencies, introducing additional relationships that improve clustering results. However, a too-low threshold may introduce noise, especially in systems with a low commits-to-entities ratio. On the other side, higher thresholds reduce noise but may exclude valuable dependencies and decrease coverage.

The optimal threshold may vary depending on system characteristics, so it is important to consider factors such as the number of commits, the size of the system, or whether it is used in combination with SD, when selecting the filtering settings.


\section{Conclusion}
\label{sec:conclusion}
Our findings indicate that using logical dependencies improves software clustering. Integrating LD with SD achieved higher MQ and MoJoFM values across all clustering algorithms and projects compared to the SD-only approach, indicating enhanced clustering due to the additional information provided by logical dependencies.

Using logical dependencies alone achieved the highest MQ and MoJoFM values at certain strength thresholds when compared with SD-only and SD+LD combinations. However, these strength thresholds corresponded to low system coverage, meaning that a significant portion of software entities was excluded from the clustering process. This suggests that logical dependencies alone may not be sufficient when full system coverage is required.

The impact of using logical dependencies is influenced by the filtering thresholds applied during their extraction. Lower strength thresholds (10\% to 40\%) provided a good trade-off between including meaningful co-change relationships and minimizing noise. At these thresholds, the combined SD+LD approach outperformed the SD-only approach. At higher strength thresholds (e.g., 100\%), the reduced coverage and increased overlap between LD and SD minimized the benefits of using logical dependencies.

This highlights the importance of selecting appropriate filtering thresholds for logical dependencies. By using filtering settings according to the characteristics of the software system, users can benefit from logical dependencies to improve architectural reconstruction.

In conclusion, logical dependencies, when appropriately filtered and combined with structural dependencies, enhance software clustering results while maintaining full system coverage. This integration helps improve system understanding and maintainability. In cases where structural dependencies are unavailable, logical dependencies alone offer a good alternative. Our future work will involve exploring automated methods for determining the optimal strength filter threshold for different systems.
